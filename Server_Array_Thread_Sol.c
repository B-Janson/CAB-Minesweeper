...
void Receive_Array_Int_Data(int socket_identifier) {
    int number_of_bytes, running = 1;
    uint16_t statistics;
	...
	while (running) {
		...
		if (send(socket_identifier, "All of array data sent by server\n", 40 , 0) == -1)
				perror("send");
	}
	free(results);
	// return results;
...
int main(int argc, char *argv[]) {
	...
	while(1) {  /* main accept() loop */
		...
		pthread_create(&client_thread, &attr, Receive_Array_Int_Data, new_fd);
